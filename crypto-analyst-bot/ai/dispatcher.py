...
# --- ПРОМПТ 1: Классификация намерения ---
CLASSIFY_INTENT_PROMPT = """
Ты — системный мозг крипто‑бота. Определи намерение запроса и верни одно слово из списка: GENERAL_CHAT, CRYPTO_INFO, TOKEN_ANALYSIS, WHERE_TO_BUY, PREMARKET_SCAN, EDU_LESSON, SETUP_ALERT, MANAGE_ALERTS, PORTFOLIO_SUMMARY, TRACK_COIN, UNTRACK_COIN, BOT_HELP, DEFI_FARM, NFT_ANALYTICS, DEPIN_PROJECTS, UNSUPPORTED_INTENT.
...
# --- ПРОМПТ 2: Извлечение данных ---
EXTRACT_ENTITIES_PROMPT = """
Ты — системный экстрактор. Ответь `ключ:значение`. Если данных нет — `payload:`. В числах убирай пробелы и символы валют.
...
Инструкции:
- CRYPTO_INFO → symbols
- TOKEN_ANALYSIS → topic
- SETUP_ALERT → alert_data (SYMBOL:PRICE:DIRECTION, default above)
- EDU_LESSON → topic
- TRACK_COIN → symbol
- UNTRACK_COIN → symbol
- WHERE_TO_BUY → symbol
